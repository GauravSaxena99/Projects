...
3. Socket Programming. - ServerSocket class is used to make an class server and another one is socket class which is for client
 */
...
import java.util.*;
import java.text.*;
import java.net.*; //for enabling socket programming
import java.io.*;
...
public class Server implements ActionListener {
   ...
   Server() //Because we need to show the frame instantly after class runs
   {
       ...
       a1 = new JPanel();
       a1.setLayout(null);
       a1.setBounds(0,70,450,555); // panel where msg lies
        
       
       text1 =  new JTextField();
       text1.setBounds(9,510,310,40);
       text1.setFont(new Font("SANS_SERIF",Font.PLAIN,16));    //typing field
       a1.add(text1);
...
// ...
    ...
    f.setLocation(200,50);    //used to change the location from its origin
   ...
   public static JPanel formatLabel(String out)
   {
    ...
    {
        new Server(); //Anonymous object (We can create normal object too)
       try{
        
            ServerSocket skt = new ServerSocket(6001); //it will run infinte times
            while(true)
            {  
                Socket s = skt.accept(); // to accept the messages infintely
